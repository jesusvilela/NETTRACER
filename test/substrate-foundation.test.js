...
test("substrate foundation exposes non-destructive V1-to-latest traffic basis", () => {
  ...
  assert.ok(result.circulation.routes.length >= 5);
  ...
  assert.equal(result.telos_field.ascent.length, 5);
  assert.ok(result.telos_field.beings.length >= 5);
  assert.ok(result.telos_field.virtuous_cycles.length >= 3);
